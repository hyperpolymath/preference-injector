...
* link:#core[Core]
** link:#preferenceinjector[PreferenceInjector]
* link:#providers[Providers]
** link:#memoryprovider[MemoryProvider]
** link:#fileprovider[FileProvider]
** link:#envprovider[EnvProvider]
** link:#apiprovider[ApiProvider]
...
** link:#react-integration[React]
** link:#express-integration[Express]
...
`+providers+`: Array of preference providers - `+conflictResolution+`:
Strategy for resolving conflicts - `+enableCache+`: Enable caching
(default: false) - `+cacheTTL+`: Cache TTL in milliseconds (default:
3600000) - `+enableValidation+`: Enable validation (default: false) -
...
*Parameters:* - `+key+`: Preference key - `+options+` (optional): -
`+defaultValue+`: Default value if not found - `+decrypt+`: Decrypt
encrypted values - `+useCache+`: Use cached value (default: true)
...
*Parameters:* - `+key+`: Preference key - `+value+`: Preference value -
`+options+` (optional): - `+priority+`: Override default priority -
`+ttl+`: Time-to-live in milliseconds - `+encrypt+`: Encrypt the value -
...
*Events:* - `+PreferenceEvent.CHANGED+`: Preference value changed -
...
*Config:* - `+filePath+`: Path to preferences file - `+priority+`:
Provider priority (default: NORMAL) - `+watchForChanges+`: Watch file
for changes (default: false) - `+format+`: File format - '`json`' or
'`env`' (auto-detected)
...
*Config:* - `+prefix+`: Environment variable prefix (default: ’’) -
`+priority+`: Provider priority (default: HIGHEST) - `+parseValues+`:
Parse values to appropriate types (default: true)
...
*Config:* - `+baseUrl+`: Base URL for API - `+apiKey+`: API
authentication key (optional) - `+headers+`: Custom headers (optional) -
`+priority+`: Provider priority (default: NORMAL) - `+timeout+`: Request
timeout in ms (default: 5000) - `+retries+`: Number of retry attempts
(default: 3)
...
*Options:* - `+injector+`: PreferenceInjector instance -
...
*Routes:* - `+GET /preferences+` - Get all preferences -
`+GET /preferences/:key+` - Get single preference -
`+PUT /preferences/:key+` - Set preference -
`+DELETE /preferences/:key+` - Delete preference -
`+HEAD /preferences/:key+` - Check if exists - `+DELETE /preferences+` -
Clear all - `+GET /preferences/_audit+` - Get audit log
...
link:../src/types/index.ts[src/types/index.ts].
